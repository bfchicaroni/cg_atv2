...
void Bullets::update(Bar &bar, const GameData &gameData, float deltaTime) {
  ...
  if (gameData.m_input[gsl::narrow<size_t>(Input::Fire)] &&
    ...
    if (bar.m_bulletCoolDownTimer.elapsed() > 250.0 / 1000.0) {
      bar.m_bulletCoolDownTimer.restart();

      // Bullets are shot in the direction of the bar's forward vector
      auto const forward{glm::rotate(glm::vec2{0.0f, 1.0f}, bar.m_rotation)};
      auto const right{glm::rotate(glm::vec2{1.0f, 0.0f}, bar.m_rotation)};
      auto const cannonOffset{(11.0f / 15.5f) * bar.m_scale};
      auto const bulletSpeed{2.0f};

      Bullet bullet{.m_dead = false,
                    .m_translation = bar.m_translation + right * cannonOffset,
                    .m_velocity = bar.m_velocity + forward * bulletSpeed};
      m_bullets.push_back(bullet);

      bullet.m_translation = bar.m_translation - right * cannonOffset;
      m_bullets.push_back(bullet);

      // Moves bar in the opposite direction
      bar.m_velocity -= forward * 0.1f;
    }
  }

  for (auto &bullet : m_bullets) {
    bullet.m_translation -= bar.m_velocity * deltaTime;
